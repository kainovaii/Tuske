package lu.kainovaii.tuske.manager.customenchantment;
...
public class EnchantConfig {
	...
	public static File file = new File(Main.getInstance().getDataFolder(), "enchantments.yml");
			...
			try {
				first: for (String enchants: y.getConfigurationSection("Enchantments").getKeys(false)){
					...
					if (cs.getString("Name") != null){
						if (CustomEnchantment.getByID(enchants) != null || CustomEnchantment.getByName(cs.getString("Name")) != null){
							Main.log("Couldn't register the enchantment '" + enchants + "' because it has a repeated name or lore name.", Level.WARNING);
							continue first;
						}
						...
						if (cs.isSet("MaxLevel") && (!cs.isInt("MaxLevel") || (cs.getInt("MaxLevel") <= 0 || cs.getInt("MaxLevel") > MAX_LEVEL))){
							Main.log("The value of 'MaxLevel' from enchantment '" + enchants + "' is not between 1 and "+ 20 +" or is not a valid value. Using the default value (" + DEFAULT_NUMBER + ").", Level.WARNING);
						...
						if (cs.isSet("Rarity") && (!cs.isInt("Rarity") || (cs.getInt("Rarity") <= 0 || cs.getInt("Rarity") > 5))){
							Main.log("The value of 'Rarity' from enchantment '" + enchants + "' is not between 1 and 5 or is not a valid value. Using the default value (" + DEFAULT_NUMBER + ").", Level.WARNING);
						...
						if (cs.isSet("AcceptedItems")){
							if (cs.getString("AcceptedItems").contains(",") || cs.getString("AcceptedItems").contains(" and ")){
								...
								for (int x = 0; x < list.size(); x++){
									if (!AcceptedItems.isValue(list.get(x).toUpperCase())){
										...
										if (list.size() < 1) {
											...
											Main.log("There aren't any valid values for 'AcceptedItems'. Using the default value ("+ WordUtils.capitalize(ACCEPTED_ITEMS.name().toLowerCase()) +")", Level.WARNING);
										} else
											Main.log("'" + list.get(x) + "' is not a valid value for 'AcceptedItems'. Ignoring it...", Level.WARNING);
									}
								...
								Main.log("The value of 'AcceptedItems' from enchantment '" + enchants + "' is not a valid value. Using the default value (" + WordUtils.capitalize(ACCEPTED_ITEMS.name().toLowerCase()) + ").");
						...
						Main.log("The enchantment '" + enchants + "' doesn't have a correct name. Skipping it.", Level.WARNING);
				}
				...
				if (!y.isInt("Config.MaxEnchantmentsPerItem")){
					if (y.isInt("Config.MaxEnchantsPerItem"))
						...
						Main.log("Wrong value for 'MaxEnchantPerItem'. Using the default value (4).", Level.WARNING);
					y.set("Config.MaxEnchantmentsPerItem", 4);
					save();
				}
				if (!y.isBoolean("Config.CompatibilityMode")){
					y.set("Config.CompatibilityMode", false);
					Main.log("Wrong value for 'CompatibilityMode'. Using the default value (false).", Level.WARNING);
				}
				if (!y.isInt("Config.GlobalRarity")){
					...
					y.set("Config.GlobalRarity", 5);
					Main.log("Wrong value for 'GlobalRarity', only accepts integers between 1 and 5. Using the default value (5).", Level.WARNING);
					
				}
					
			} catch (Exception e){
				Main.log("A error has occured when trying to load the enchantments file. Checks if hasn't any wrong values. \n" + e, Level.SEVERE);
			}
		}
	}
	public static boolean loadFile(){
		if (file.exists()){
			try {
				...
				Main.log("Couldn't load the enchantments file, probably there are wrong values. But if you think everything is right, ask for support using this message and the enchantment file. \n\n" + e.getMessage(), Level.SEVERE);
			}
		}
		return false;
	}
	public static void createNewFile(){
		if (!file.exists())
			try {
				Main.getInstance().saveResource("enchantments.yml", false);
			} catch(Exception e){
				Main.log("Error occurred when creating a new enchantment file: \n" + e.getMessage(), Level.SEVERE);
	...
	public static void save() {
		if (file.exists())
			try {
				...
				Main.log("Error occurred when saving enchantment file: \n" + e.getMessage(), Level.SEVERE);
	...
	public static void reload(){
		file = new File(Main.getInstance().getDataFolder(), "enchantments.yml");
