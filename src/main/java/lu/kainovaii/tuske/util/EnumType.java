package lu.kainovaii.tuske.util;
...
public class EnumType extends ClassInfo{
	...
	public <T extends Enum<T>> EnumType(final Class<T> c, String code, String regexUser, String languageNode) {
		...
		try {
			...
			if (!Main.getInstance().getConfig().isSet("cancel_override_type." + code) && enumUtils != null) {
				...
				Main.debug("Couldn't register the type '" + code + "'. Due to: " + (e.getMessage() != null && !e.getMessage().isEmpty() ? e.getMessage() : "unknown"));
		}

	}
	public static <T extends Enum<T>> Parser<T> getParser(Class<T> c, EnumUtils<T> enumUtils) {
		if (!Main.getInstance().getConfig().getBoolean("use_only_enum_names")){//It will parse the enum types as '<Enum name>' and '<Enum type>.<Enum name>'
			return new Parser<T>() {
				@Override
				@Nullable
